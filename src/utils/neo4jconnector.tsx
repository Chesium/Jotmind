...
export async function retrieveInfo(session: Session): Promise<NodeData[]> {
  ...
  if (res === undefined) {
    ...
    console.log(people);
    return people.map((p) =>
      PersonNodeDataToCy(p, testTagSignature, "expanded")
    );
  }
}

export async function retrieveInfoAsMap(
  session: Session
): Promise<PersonNodeMap> {
  const varname = "u";
  var res = await query(session, `MATCH (${varname}:Person) RETURN ${varname}`);
  if (res === undefined) {
    console.log("ERR: res is undefined");
    ...
    return people.reduce((acc, cur) => ({ ...acc, [cur.elementId]: cur }), {});
  }
}
