...
/**
 * pgvector `vector` column (US-021). Declared without a fixed dimension so a
 * single table can hold embeddings of whatever size the configured provider
 * produces (the mock provider is 8-dim; real providers vary). Values are
 * marshalled to/from pgvector's text format (`[1,2,3]`). Exact nearest-neighbor
 * search via the `<=>` operator works without a fixed-dimension index, which is
 * sufficient for V1's per-Knowledge-Base scale.
 */
...
/**
 * Lifecycle status of the derived Apache AGE graph projection (US-026). A single
 * global row (`id = 'default'`, enforced by a check constraint) records whether
 * the projection is `synchronized`, currently `rebuilding`, or `failed`, plus
 * the triggering rebuild job + timestamps + last error so projection lag/failure
 * is visible in the status/job UI (AC7). The AGE graph itself is a derived index
 * — this table never holds canonical data, so a failed projection can never
 * corrupt canonical records.
 */
export const graphProjectionStatus = pgTable(
  'graph_projection_status',
  {
    id: text('id').primaryKey().default('default'),
    state: text('state').notNull().default('synchronized'),
    activeKnowledgeBaseId: uuid('active_knowledge_base_id').references(() => knowledgeBases.id, {
      onDelete: 'set null',
    }),
    lastJobId: uuid('last_job_id').references(() => jobs.id, { onDelete: 'set null' }),
    lastRebuildStartedAt: timestamp('last_rebuild_started_at', { withTimezone: true }),
  ...
  (table) => ({
    singletonCheck: check('graph_projection_status_singleton_check', sql`${table.id} = 'default'`),
    stateCheck: check(
      'graph_projection_status_state_check',
      sql`${table.state} IN ('rebuilding', 'synchronized', 'failed')`,
    ),
  }),
);

export type GraphProjectionStatusRow = typeof graphProjectionStatus.$inferSelect;
export type NewGraphProjectionStatusRow = typeof graphProjectionStatus.$inferInsert;

/**
 * Vector embeddings store (US-021). Embeddings are generated by background
 * indexing jobs and stored in PostgreSQL via `pgvector` (AC1). One row per
 * (canonical record, model): `target_type` + `target_id` identify the indexed
 * entity/claim/note/source, `content_hash` lets re-indexing skip unchanged
 * content, and `embedding` holds the vector. Existing rows can power vector
 * search even when the generation provider is currently unavailable (AC4).
 * Not soft-deleted — embeddings are a derived index, rebuilt from canonical
 * records; when a record is soft-deleted its embedding row is removed.
 */
export const embeddings = pgTable(
  ...
  (table) => ({
    targetTypeCheck: check(
      'embeddings_target_type_check',
      sql`${table.targetType} IN ('entity', 'claim', 'note', 'source')`,
    ),
    dimensionsCheck: check('embeddings_dimensions_check', sql`${table.dimensions} > 0`),
    byTarget: uniqueIndex('embeddings_target_model_uq').on(
      table.targetType,
      table.targetId,
      table.model,
    ),
    byKb: index('embeddings_kb_idx').on(table.knowledgeBaseId),
  }),
);
